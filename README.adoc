...
NOTE: The Appsody version <0.5 lacks the feature of direct pushing the image to the registry and set the correctly the image URL in the manifest file. However the Appsody version >0.5 has an other link:https://github.com/appsody/appsody/issues/827[issue] which is not usable in this walkthrough. For this, please call the `deploy` command, adjust the manifest file, and re-apply the manifest file again.
